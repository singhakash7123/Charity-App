...
const Payment = () => {
  ...
  const RenderPaymentType = paymenttype.map((payitem, index) => {
    return (
      <button
        onClick={() => { setPaytype(payitem) }}
        key={index}
        className={`flex-1 text-center py-2 border-2 border-blue-400 rounded-xl font-semibold text-[16px] ${paytype === payitem ? 'bg-blue-300 text-white' : 'bg-white'}`}
      >
        {payitem}
      </button>
    )
    ...
    <section className='min-h-screen bg-gradient-to-tr from-blue-100 to-blue-200 flex flex-col items-center py-8 '>
      ...
      <div className='max-w-[600px] w-full mt-15 '>
        <h3 className='flex justify-center'>
          <span className='text-3xl p-4 rounded-full text-white bg-gradient-to-br from-blue-500 to-blue-600'>
            <FaHeart />
          </span>
        </h3>
        <h5 className='text-center text-[14px] mt-5'>Make a Difference Today</h5>
        <h6 className='text-center text-[14px] mt-1 px-5'>Your generous donation helps us continue our mission and create lasting impact</h6>
      </div>
...
>>>>>>> f013ea4be1ecc5cbcb54f9511b61cce58239bb53
          <br />
      ...
      <div className='max-w-[600px] w-full mt-7 p-5 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 '>
        ...
        <button onClick={handleDonate} className='flex w-full justify-center font-medium bg-white py-1 text-purple-500 rounded-lg mt-5'>
          Complete Donation
        </button>
      </div>

      <p className='text-center text-gray-400 mt-6'>By completing this donation you agree to our terms and condition</p>
    </section>
  )
}
